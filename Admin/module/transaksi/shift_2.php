...
<section role="main" class="content-body">
	...
	$jumlah1 = mysqli_query($query, "SELECT COUNT(*) FROM detail_transaksi dt JOIN jual j ON dt.no_nota = j.no_nota WHERE (j.tanggal = '$currentDate') AND (j.waktu BETWEEN '16:00:00' AND '22:00:00')");
	$out_jumlah = mysqli_fetch_array($jumlah1);
	$jumlah = $out_jumlah[0];

	$proces1 = mysqli_query($query, "SELECT COUNT(*) FROM jual j WHERE (j.tanggal = '$currentDate') AND (j.waktu BETWEEN '16:00:00' AND '22:00:00') AND (status = 'Process')");
	$out_proces = mysqli_fetch_array($proces1);
	$proces = $out_proces[0];

	$selesai1 = mysqli_query($query, "SELECT COUNT(*) FROM jual j WHERE (j.tanggal = '$currentDate') AND (j.waktu BETWEEN '16:00:00' AND '22:00:00') AND (status = 'Success')");
	$out_selesai = mysqli_fetch_array($selesai1);
	$done = $out_selesai[0];

	$omset1 = mysqli_query($query, "SELECT SUM(total) FROM jual j WHERE (j.tanggal = '$currentDate') AND (j.waktu BETWEEN '16:00:00' AND '22:00:00') AND (status = 'Success')");
	...
	<section class="panel">
		<header class="panel-heading">
			...
			<h2 class="panel-title">Transaksi Shift 2</h2>
		</header>
		<div class="panel-body">
			<!-- Nav tabs -->
							...
							<tbody>
								<?php
									...
									$hasil = mysqli_query($query, "SELECT dt.no_nota, s.Nama, j.tanggal, j.waktu, j.nama_pembeli, p.nama_powder, sj.nama_penyajian, t.nama_topping, dt.jumlah, j.status 
												   ...
												   WHERE (j.tanggal = '$currentDate') AND (j.waktu BETWEEN '16:00:00' AND '22:00:00') AND dt.id_region = " . $serves['id_region']);
